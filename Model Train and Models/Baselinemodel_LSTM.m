function [layers] = LSTM_based_model(kernelsize)
...
layers1 = [
    ...
    lstmLayer(numFilters*1,'OutputMode','sequence')
    swishLayer
    
    lstmLayer(numFilters*2,'OutputMode','sequence')
    swishLayer
    
    lstmLayer(numFilters*4,'OutputMode','sequence')
    swishLayer

    swishLayer(Name="branch1")                           % X
    ];
...
layers2 = [
    ...
    fullyConnectedLayer(64,'Name','kv_fc1282')
    ...
    functionLayer(@(X) cbtocbt(X),Formattable=true, Acceleratable=false, Name = "featurelast1")
    ];
layers = addLayers(layers,layers2);

layers3 = [
    functionLayer(@(x,y) x+y, Name = "addition") % additionLayer(2,"Name","addition")

    % fullyConnectedLayer(numFilters*6,'Name','dec1_ups')
